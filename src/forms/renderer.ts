...
// Preset utilities for easier integration

/**
 * Run a form with preset loading capability
 */
export async function runFormWithPresets(
  form: Form,
  options = {},
  showPresetPrompt: boolean = true,
): Promise<{ answers: Record<string, any>; usedPreset?: string }> {
  if (!showPresetPrompt) {
    const answers = await runForm(form, options)
    return { answers }
  }

  // Import preset forms dynamically to avoid circular deps
  const { createPresetLoadForm } = await import('./preset-forms')
  const { defaultPresetManager } = await import('./preset-manager')

  try {
    const presetLoadForm = await createPresetLoadForm(form.id)
    const presetChoice = await runForm(presetLoadForm, { validateOnChange: true })

    if (presetChoice.selectedPreset && presetChoice.selectedPreset !== 'none') {
      // Load preset and run form with pre-filled answers
      const preset = await defaultPresetManager.getPreset(presetChoice.selectedPreset)
      if (preset) {
        // Create engine with preset answers
        const engine = new FormEngine(form, options)
        await engine.loadPreset(preset.id)

        const renderer = new FormRenderer(engine)
        const answers = await renderer.run()

        return { answers, usedPreset: preset.id }
      }
    }
  } catch (error) {
    console.log('\n⚠️  Preset loading failed, starting fresh...')
  }
...
/**
 * Simple helper to save form answers as a preset after completion
 */
export async function saveFormAsPreset(
  formId: string,
  ...
  try {
    ...
    const preset = await defaultPresetManager.savePreset(presetData)
